      ...
      <Layout>
        ...
        <div className="flex flex-col xl:flex-row gap-8 bg-white dark:bg-[#18191e] text-slate-900 dark:text-white p-2 rounded-[2rem] border border-slate-200 dark:border-slate-800/40 relative overflow-hidden font-sans transition-colors duration-200">
          ...
          <div className="flex-1 p-2 sm:p-4 space-y-6">
                      ...
                      <BarChart data={progressChartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        ...
                        <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                          {progressChartData.map((_, index) => (
                            <Cell
                              key={`cell-${index}`}
                              fill={index % 3 === 0 ? '#ff3b30' : index % 3 === 1 ? '#ff9500' : '#ffcc00'}
                            />
                          ))}
                        </Bar>
                      </BarChart>
                ...
                <div>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">Tasks breakdown by phase</p>
                    </div>
                  ...
                  <div className="mt-3 mb-4">
                    <div className="flex justify-between text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">
                      ...
                      <span className="text-[#ff3b30]">{stats.teamProgressPercent}%</span>
                    </div>
                    <div className="w-full bg-slate-100 dark:bg-[#18191e] h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#ff3b30] h-full rounded-full transition-all duration-300" style={{ width: `${stats.teamProgressPercent}%` }}></div>
                    </div>
                  </div>
                </div>
            ...
            {/* ── Row 3: Project Timelines + Team Progress ── */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

              {/* Project Timeline Capsules */}
                    ...
                    {projectTimelines.map((proj) => (
                      <div key={proj.id} className="relative h-6 flex items-center group">
                        <div
                          ...
                          style={{ left: proj.startPercent, width: proj.widthPercent, backgroundColor: proj.color }}
                          title={proj.name}
                        >
                ...
                <div className="space-y-4 pt-3">
                  ...
                  <div>
                    <div className="flex justify-between text-[9px] text-slate-500 dark:text-slate-450 uppercase tracking-wider mb-1.5 font-bold">
                      ...
                      <span className="text-[#ff9500] font-extrabold">
                        {allProjects.length > 0 ? `${Math.round((allProjects.filter(p => (p.progress || 0) >= 50).length / allProjects.length) * 100)}%` : '0%'}
                      </span>
                    </div>
                    <div className="w-full bg-slate-100 dark:bg-[#18191e] h-1.5 rounded-full overflow-hidden">
                      <div
                        className="bg-[#ff9500] h-full rounded-full"
                        style={{ width: allProjects.length > 0 ? `${Math.round((allProjects.filter(p => (p.progress || 0) >= 50).length / allProjects.length) * 100)}%` : '0%' }}
                      ></div>
                    </div>
                  </div>
                </div>
                    ...
                    <div key={task.id} className="text-[10px] bg-slate-50 dark:bg-[#1e1f25]/50 border border-slate-200 dark:border-slate-850 p-2.5 rounded-xl space-y-1 hover:border-slate-300 dark:hover:border-slate-800 transition-colors text-slate-800 dark:text-slate-200">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wide ${
                          task.priority === 'HIGH' ? 'bg-red-50 dark:bg-red-950/20 text-[#ff3b30]' :
                          task.priority === 'MEDIUM' ? 'bg-amber-50 dark:bg-amber-950/20 text-[#ff9500]' :
